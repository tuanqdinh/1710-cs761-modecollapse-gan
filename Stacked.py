...
class DataSampler(object):
    def __init__(self):
        ...
        for i in range(1000):
            b = self.labeled_batch(10000,i)
            imgs = np.concatenate((imgs, b[0]),axis = 0)
            labs = np.concatenate((labs, b[1]),axis = 0)
        self.imgs = imgs
        self.labs = labs

    def next_batch(self,size):
        ind = np.random.randint(self.labs.shape[0],size = size)
        return self.imgs[ind,:], self.labs[ind,:]
