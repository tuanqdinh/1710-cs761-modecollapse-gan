...
class GGAN:
    def __init__(self, im_size, model_name):
        ...
        self.G_W1 = tf.Variable(initializer(shape=[self.dim_z, self.dim_h]), name='G_W1')
        self.G_b1 = tf.Variable(tf.zeros(shape=[self.dim_h]), name='G_b1')

        self.G_W2 = tf.Variable(initializer([self.dim_h, self.dim_x_2]), name='G_W2')
        self.G_b2 = tf.Variable(tf.zeros(shape=[self.dim_x_2]), name='G_b2')

        self.G_W3 = tf.Variable(initializer([self.dim_x_2, self.dim_x]), name='G_W3')
        self.G_b3 = tf.Variable(tf.zeros(shape=[self.dim_x]), name='G_b3')
        ...
        self.D_W1 = tf.Variable(initializer([self.dim_x, self.dim_h]), name='D_W1')
        self.D_b1 = tf.Variable(tf.zeros(shape=[self.dim_h]), name='D_b1')
        self.D_W2 = tf.Variable(initializer([self.dim_h, 1]), name='D_W2')
    ...
    def get_next_w(self, W):
        eta = 0.01
        v = tf.random_normal(tf.shape(W))
        x = tf.reshape(v, [-1])
        # from IPython import embed; embed()
        n = tf.norm(x, ord='euclidean')
        normed_v = tf.scalar_mul(1/n, v)
        return W + eta * normed_v

    def climb_weight(self):
        # flatten weights
        alpha = 0.0
        m = 1000
        min_loss = 1000.0
        ...
        for i in range(m):
            ...
            self.G_W1 = self.get_next_w(self.G_W1)
            # careful here
            gsample = self.generator(self.Z)
            dfake, dlogit_fake = self.discriminator(gsample)
            gloss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                logits=dlogit_fake, labels=tf.ones_like(dlogit_fake)))
            self.theta_G = tf.cond(tf.less(min_loss + alpha, gloss), lambda:self.theta_G, lambda:theta_G2)
            # do not need to roll back
            self.G_W1 = self.theta_G[0]

            min_loss = tf.cond(tf.less(min_loss + alpha, gloss), lambda:min_loss, lambda:gloss)
            # break - simplest hill climbing

        return min_loss

    def get_steepest_point(self):
        # flatten weights
        alpha = 0.0
        m = 1000
        min_loss = 1000.0
        ...
        for i in range(m):
            # vars['matrix1:0'].assign_add(tf.constant(1))
            self.G_W1 = self.get_next_w(self.G_W1)
            self.G_W2 = self.get_next_w(self.G_W2)
            self.G_W3 = self.get_next_w(self.G_W3)
            self.G_b1 = self.get_next_w(self.G_b1)
            self.G_b2 = self.get_next_w(self.G_b2)
            self.G_b3 = self.get_next_w(self.G_b3)
            theta_G2 = [self.G_W1, self.G_W2, self.G_W3,
                            self.G_b1, self.G_b2, self.G_b3]
            # careful here
            gsample = self.generator(self.Z)
            dfake, dlogit_fake = self.discriminator(gsample)
